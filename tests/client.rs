...
fn get_latest_any_news_data() {
    let _ = tokio_test::block_on(Client::new().get_news_latest(stream::NewsSubject::Any, 3));
}

#[test]
fn get_latest_global_news_data() {
    let _ = tokio_test::block_on(Client::new().get_news_latest(stream::NewsSubject::Global, 3));
}
...
fn panic_if_invalid_limit_range_in_getting_latest_news() {
    let _ = tokio_test::block_on(Client::new().get_news_latest(stream::NewsSubject::Any, 0));
}
