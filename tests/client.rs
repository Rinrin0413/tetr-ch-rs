...
fn get_latest_global_news_data() {
    let _ = tokio_test::block_on(Client::new().get_news_latest(NewsStreamParam::Global, 3));
}
...
fn panic_if_invalid_limit_range_in_getting_latest_news() {
    let _ = tokio_test::block_on(Client::new().get_news_latest(NewsStreamParam::Global, 0));
}
